...
def apply_filters(data, fs, notch_freq=50):
    """
    Applies a 1-15Hz Bandpass and a Notch filter to a data array.
    Uses a larger pad to avoid edge artifacts.
    """
    if len(data) < 100:  # Not enough data to filter reliably
        return data
    ...
    # 1. Notch Filter
    if notch_freq:
        b_notch, a_notch = iirnotch(notch_freq / nyq, 30.0)
        data = filtfilt(b_notch, a_notch, data)
    
    # 2. Bandpass Filter (1-15Hz)
    low = 1.0 / nyq
    high = 15.0 / nyq
...
def eeg_detector_loop():
    ...
    try:
        ...
        # Internal raw buffer for stable filtering (2 seconds of data)
        raw_processing_buffer = deque(maxlen=int(fs * 2))
        blink_active = False

        while detector_running:
            # Pull all available data since last check
            ...
            # We need at least 1 second of data to filter stably
            ...
            # Update the UI signal buffer (for plotting)
            with state_lock:
                # We only take the latest filtered samples to keep the plot moving
                new_filtered_samples = filtered_window[-len(raw_chunk):]
                for s in new_filtered_samples:
                    signal_buffer.append(float(s))
                
                curr_threshold = threshold_uv
                curr_cooldown = cooldown_secs

            # --- BLINK DETECTION LOGIC ---
            # Use the most recent filtered samples for detection
            recent_samples = filtered_window[-len(raw_chunk):]
            for val in recent_samples:
                abs_val = abs(val)
                now = time.time()

                # Trigger Blink
                if abs_val > curr_threshold and not blink_active:
                    if (now - last_blink_ts) > curr_cooldown:
                        blink_active = True
                        with state_lock:
                            last_blink_ts = now
                            blink_pending = True
                ...
                # Reset Blink (Schmitt Trigger logic)
                elif blink_active and abs_val < (curr_threshold * BLINK_OFF_FACTOR):
                    blink_active = False
